...
def main(random_state=42):
    ...
    if exist_ignore or (not os.path.exists(f".cache/feature_df_capping{capping_name}_MRTs.parquet")):
        ...
        result_df.to_parquet(f".cache/feature_df_capping{capping_name}_MRTs.parquet")
    else:
        result_df = pd.read_parquet(f".cache/feature_df_capping{capping_name}_MRTs.parquet")
    ...
    capping_group = "[*]C"
    excepted_mol_num = excepted_mol_num
    ...
    if exist_ignore or (not os.path.exists(f".cache/CKAs_feature_df_capping{capping_name}_MRTs.parquet")):
        # Generate the feature DataFrame
        test_result_df = gen_feature_df_paral(test_exp_worksheet, test_mol_descriptors_rev, feature_list, 
                                            capping_group=capping_group, expected_mol_num=excepted_mol_num, 
                                            r_flag=r_flag, sm_flag=sm_flag, ts_flag=ts_flag)
        # Save the result to a parquet file
        test_result_df.to_parquet(f".cache/CKAs_feature_df_capping{capping_name}_MRTs.parquet")
    else:
        test_result_df = pd.read_parquet(f".cache/CKAs_feature_df_capping{capping_name}_MRTs.parquet")
