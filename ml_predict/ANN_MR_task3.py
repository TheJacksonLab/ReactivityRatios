...
from utils.model import train_ANN_model_regressor

# Add a predict function for PyTorch models
def predict_pytorch(model, X, device='cpu'):
...
def main(random_state=42):
    ...
    best_model = train_ANN_model_regressor(
        X_train, y_train_log, 
        X_val=X_test, y_val=y_test_log,
        params=ANN_params,
        random_state=random_state,)[0]

    y_train_log_pred = predict_pytorch(best_model, X_train)
    y_test_log_pred = predict_pytorch(best_model, X_test)
    ...
    y_test_cka_log_pred = predict_pytorch(best_model, X_test_cka_scaled)
    ...
    def print_model_parameters(model):
        ...
        # # Optional: Print size in MB (assuming float32/4 bytes per parameter)
        # size_mb = total_params * 4 / (1024 * 1024)
        # print(f"Model size (MB): {size_mb:.2f}")
        
    print_model_parameters(best_model)
