...
def main(random_state=42):
    mol_descriptors_path = "/Users/jingdan/Desktop/scrugs/project/0418_capping/wb97xd_DFT_cappingC.parquet"
    ...
    if exist_ignore or (not os.path.exists(f".cache/feature_df_capping{capping_name}_wb97xd.parquet")):
        ...
        result_df.to_parquet(f".cache/feature_df_capping{capping_name}_wb97xd.parquet")
    else:
        result_df = pd.read_parquet(f".cache/feature_df_capping{capping_name}_wb97xd.parquet")
    ...
    test_mol_descriptors_path = "/Users/jingdan/Desktop/scrugs/project/0418_capping/wb97xd_DFT_CKAs_cappingC.parquet"
    unrelated_descriptors = ["react_atom_serial", ]
    ...
    if exist_ignore or (not os.path.exists(f".cache/CKAs_feature_df_capping{capping_name}_wb97xd.parquet")):
        # Generate the feature DataFrame
        test_result_df = gen_feature_df_paral(test_exp_worksheet, test_mol_descriptors_rev, feature_list, 
                                            capping_group=capping_group, expected_mol_num=excepted_mol_num)
        # Save the result to a parquet file
        test_result_df.to_parquet(f".cache/CKAs_feature_df_capping{capping_name}_wb97xd.parquet")
    else:
        test_result_df = pd.read_parquet(f".cache/CKAs_feature_df_capping{capping_name}_wb97xd.parquet")
