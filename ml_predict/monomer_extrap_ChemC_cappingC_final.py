...
def main(monomer_name):
    capping_group = "[*]C" 
    feature_list = feature_dict["dft_FMO_size_shape"]
    r_low = 0.0
    r_high = 50.
    ...
    # mol_descriptors_path = f"/home/jingdan/scrugs/project/0418_capping/DFT_capping{capping_name}.parquet"
    ...
    hash_value = str(monomer_name)

    
    if exist_ignore or (not os.path.exists(f".cache/feature_df_capping{capping_name}_monextrap_final.parquet")):
        ...
        result_df.to_parquet(f".cache/feature_df_capping{capping_name}_monextrap_final.parquet")
    else:
        result_df = pd.read_parquet(f".cache/feature_df_capping{capping_name}_monextrap_final.parquet")
    ...
    train_flag = np.array(list(map(lambda x: monomer_name not in x.split("."), 
                                   (filtered_result_df['Monomer_1']+"."+filtered_result_df['Monomer_2']).tolist())))
    test_flag = np.logical_not(train_flag)
    
    train_indices = indices[train_flag]
    test_indices = indices[test_flag]

    # print(unpack_list_column(filtered_result_df.iloc[train_indices,:], column_name='type').columns)
...
if __name__ == "__main__":
    exp_worksheet = pd.read_csv('data/monomer_react_ratio_smarts.csv')
    all_names = exp_worksheet['Monomer_1'].tolist() + exp_worksheet['Monomer_2'].tolist()
    
    
    for monomer_name in set(all_names):
        from time import time
        t0 = time()

        try:
            _, scores = main(monomer_name)
        except Exception as e:
            print(f"Error processing {monomer_name}: {e}")
            continue
        training_accuracy = scores['training_accuracy']
        test_accuracy = scores['test_accuracy']
        f1 = scores['f1_score']
        test_log_rmse = scores['test_log_rmse']
        test_log_mae = scores['test_log_mae']
        test_log_r2 = scores['test_log_r2']
        
        
        info_dict ={"monomer_name": monomer_name, 
                    "probability":round(all_names.count(monomer_name) / len(all_names), 4), 
                    "train_accuracy": round(training_accuracy, 4), 
                    "test_accuracy": round(test_accuracy, 4), "f1": round(f1, 4), 
                    "test_log_rmse": round(test_log_rmse, 4),
                    "test_log_mae": round(test_log_mae, 4),
                    "test_log_r2": round(test_log_r2, 4)}
        
        print(f"Running with monomer_name={monomer_name}, probability={info_dict['probability']}, \
            train_accuracy={info_dict['train_accuracy']}, test_accuracy={info_dict['test_accuracy']}, \
                f1={info_dict['f1']}, test_log_rmse={info_dict['test_log_rmse']}, \
                    test_log_mae={info_dict['test_log_mae']}, test_log_r2={info_dict['test_log_r2']}")
        
        
        os.makedirs("monomer_extrap_ChemC_cappingC_final", exist_ok=True)
        jsonl_path = f"monomer_extrap_ChemC_cappingC_final/xgb_scores.jsonl"
        with open(jsonl_path, "a") as f:
            f.write(json.dumps({"info": info_dict, "hash_value": str(monomer_name)}) + "\n")
