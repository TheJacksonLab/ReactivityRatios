...
def search_smiles(smiles, worksheet, type_='int', val_set=None, include_cv=False):
    inchikey1 = Chem.MolToInchiKey(Chem.MolFromSmiles(smiles, sanitize=True))
    res_lis1 = search_inchikey(inchikey1, worksheet, type_, include_cv)
    ...
    res_lis2 = search_inchikey(inchikey2, worksheet, type_, include_cv)
    if (len(res_lis2) > 0) and ((val_set is None) or (inchikey2 in val_set)):
        return res_lis2
    return []

def process_row(idx, exp_worksheet, mol_descriptors_rev, feature_list, capping_group, expected_mol_num, include_cv=False):
    ...
    mol1_inchiks = search_smiles(mol1s, mol_descriptors_rev, 'int', include_cv=include_cv)
    mol2_inchiks = search_smiles(mol2s, mol_descriptors_rev, 'int', include_cv=include_cv)
    rad1_inchikss = search_smiles(rad1s, mol_descriptors_rev, 'int', include_cv=include_cv)
    rad2_inchiks = search_smiles(rad2s, mol_descriptors_rev, 'int', include_cv=include_cv)
...
def gen_feature_df_paral(exp_worksheet, mol_descriptors_rev, feature_list, capping_group="[*]C", expected_mol_num=4, include_cv=False):
...
def main(random_state=42):
    mol_descriptors_path = "/Users/jingdan/Desktop/scrugs/project/0418_capping/DFT_cappingC_cv.parquet"
    ...
    if exist_ignore or (not os.path.exists(f".cache/feature_df_capping{capping_name}_cv.parquet")):
        # Generate the feature DataFrame
        result_df = gen_feature_df_paral(exp_worksheet, mol_descriptors_rev, feature_list, 
                                        capping_group=capping_group, expected_mol_num=excepted_mol_num, include_cv=True)
        # Save the result to a parquet file
        result_df.to_parquet(f".cache/feature_df_capping{capping_name}_cv.parquet")
    else:
        result_df = pd.read_parquet(f".cache/feature_df_capping{capping_name}_cv.parquet")
    ...
    best_xgb_model = train_xgb_model_regressor(
        X_train, y_train_log,
        params=best_params,
    ...
    if exist_ignore or (not os.path.exists(f".cache/CKAs_feature_df_capping{capping_name}_cv.parquet")):
        # Generate the feature DataFrame
        ...
        # Save the result to a parquet file
        test_result_df.to_parquet(f".cache/CKAs_feature_df_capping{capping_name}_cv.parquet")
    else:
        test_result_df = pd.read_parquet(f".cache/CKAs_feature_df_capping{capping_name}_cv.parquet")
