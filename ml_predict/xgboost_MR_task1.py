...
def main(random_state=42):
    ...
    result_df_split = filtered_result_df[['Monomer_1', 'Monomer_2', 'r1', 'r2'] + list(filtered_result_df.columns[2:-3])]
    ...
    X_train = np.concatenate([
        unpack_list_column(result_df_split.iloc[train_indices,:], column_name=['r1', 'r2']).iloc[:, 4:].values
    ], axis=0)

    y_train = np.concatenate([
        unpack_list_column(result_df_split.iloc[train_indices,:], column_name=['r1', 'r2'])[['r1', 'r2']].values
    ], axis=0)

    X_test = np.concatenate([
        unpack_list_column(result_df_split.iloc[test_indices,:], column_name=['r1', 'r2']).iloc[:, 4:].values], axis=0)

    y_test = np.concatenate([
        unpack_list_column(result_df_split.iloc[test_indices,:], column_name=['r1', 'r2'])[['r1', 'r2']].values
    ], axis=0)
    ...
    train_label_predict = np.array(list(map(
        judge_type, y_train_pred.tolist())))
    test_label_predict = np.array(list(map(
        judge_type, y_test_pred.tolist())))
    ...
    test_cka_label = np.array(list(map(judge_type, test_result_df[['r1', 'r2']].values.tolist())))

    result_df_split = test_result_df[['Monomer_1', 'Monomer_2', 'r1', 'r2'] + list(filtered_result_df.columns[2:-3])]
    
    X_test_cka = np.concatenate([
        unpack_list_column(result_df_split.iloc[:,:], column_name=['r1', 'r2']).iloc[:, 4:].values
    ], axis=0)

    y_test_cka = np.concatenate([
        unpack_list_column(result_df_split.iloc[:,:], column_name=['r1', 'r2'])[['r1', 'r2']].values
    ], axis=0)
    y_test_log_cka = np.log(y_test_cka)

    X_test_cka_scaled = scaler.transform(X_test_cka)
    X_test_cka = X_test_cka_scaled

    y_test_cka_log_pred = best_xgb_model.predict(X_test_cka)
    y_test_cka_pred  = np.exp(y_test_cka_log_pred)

    cka_test_label_predict = np.array(list(map(
        judge_type, y_test_cka_pred.tolist())))


    test_cka_log_mse = mean_squared_error(y_test_log_cka, y_test_cka_log_pred)
    test_cka_log_rmse = np.sqrt(test_cka_log_mse)
    test_cka_log_mae = mean_absolute_error(y_test_log_cka, y_test_cka_log_pred)
    test_cka_log_r2 = r2_score(y_test_log_cka, y_test_cka_log_pred)
    ...
    test_cka_f1 = f1_score(test_cka_label, cka_test_label_predict, average='weighted')
    test_cka_accuracy = np.sum(cka_test_label_predict == test_cka_label) / len(test_cka_label)
