...
def main(random_state=42):
    ...
    # feature_list = dft_FMO + dft_surface + dft_els_prop + dft_size_shape + reac_BV_descriptors
    ...
    if exist_ignore or (not os.path.exists(f".cache/feature_df_capping{capping_name}_M.parquet")):
        ...
        result_df.to_parquet(f".cache/feature_df_capping{capping_name}_M.parquet")
    else:
        result_df = pd.read_parquet(f".cache/feature_df_capping{capping_name}_M.parquet")
    ...
    prefix_mappings = {
        ...
        "ts11_": {"split1": True, "split2": None},
        "ts12_": {"split1": True, "split2": None},
        "ts22_": {"split1": None, "split2": ("ts11_", "ts22_")},
        "ts21_": {"split1": None, "split2": ("ts12_", "ts21_")},
        "sm11_": {"split1": True, "split2": None},
        "sm12_": {"split1": True, "split2": None},
        "sm22_": {"split1": None, "split2": ("sm11_", "sm22_")},
        "sm21_": {"split1": None, "split2": ("sm12_", "sm21_")},
    }
    ...
    if exist_ignore or (not os.path.exists(f".cache/CKAs_feature_df_capping{capping_name}_M.parquet")):
        # Generate the feature DataFrame
        test_result_df = gen_feature_df_paral(test_exp_worksheet, test_mol_descriptors_rev, feature_list, 
                                            capping_group=capping_group, expected_mol_num=excepted_mol_num)
        # Save the result to a parquet file
        test_result_df.to_parquet(f".cache/CKAs_feature_df_capping{capping_name}_M.parquet")
    else:
        test_result_df = pd.read_parquet(f".cache/CKAs_feature_df_capping{capping_name}_M.parquet")
